...
func (handler *CLIYams) retryPreviusFailedUploads(threads int) error {
...
func (handler *CLIYams) Sync(threads int, imagesDumpYamsPath string) error {
	...
	handler.retryPreviusFailedUploads(threads)
...
func (handler *CLIYams) sendWorker(id int, jobs <-chan domain.Image, wg *sync.WaitGroup, previusFailedUpload bool) {
	wg.Add(1)
	defer wg.Done()
	for image := range jobs {
		err := handler.Interactor.Send(image)
		if err == nil && previusFailedUpload {
		...
		if err != nil {
			if err == usecases.ErrYamsDuplicate {
				...
				if externalChecksum != image.Metadata.Checksum {
					...
					// mark to upload in the next sync process (because cache)
				...
				handler.Interactor.SyncErrorRepo.SetErrorSync(image.Metadata.ImageName)
			}
		}
	}
}
